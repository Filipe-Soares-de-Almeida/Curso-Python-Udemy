...
def criar_arquivos(qtd: int, zip_dir: Path):
  for i in range(qtd):
    texto = 'arquivo_%s' % i  # pylint: disable=consider-using-f-string
    with open(zip_dir / f'{texto}.txt', 'w', encoding='utf-8') as arquivo_:
      arquivo_.write(texto)

criar_arquivos(10, CAMINHO_ZIP_DIR)

# Criando um zip e adicionando arquivos
with ZipFile(CAMINHO_COMPACTADO, 'w') as _zip:
...
with ZipFile(CAMINHO_COMPACTADO, 'r') as __zip:
  for arquivo in __zip.namelist():
    print(arquivo)

# Extraindo arquivos de um zip
with ZipFile(CAMINHO_COMPACTADO, 'r') as ___zip:
  ___zip.extractall(CAMINHO_DESCOMPACTADO)
